...
var messages = map[string]map[string]string{
	"zh-CN": {
		...
		"url_overrides": "URL 参数可覆写", "preset_preview": "地图预设实时预览", "preview_mode": "预览模式", "interactive_widget_preview": "交互式 Widget", "image_fallback": "Image fallback", "width": "宽度", "height": "高度", "auto_width": "按当前选项自动计算宽度",
		...
		"save_settings": "保存站点设置", "combined_widget": "一体式接入", "separate_tracker": "分离式接入", "widget_resource": "Widget", "javascript_widget": "交互式 Widget", "image_widget": "Image Widget", "recommended": "推荐", "interactive": "交互式", "compatibility_mode": "兼容模式", "image_widget_limitations": "无需 JavaScript；不创建浏览器 Visitor ID，路径默认取 Referer。图片代理、预取或缓存可能只展示地图而不计数；固定页面可在 URL 中加入 path 参数。", "tracker_resource": "Tracker", "map_resource": "交互式地图", "separated_widget_help": "只负责显示，不记录 Pageview；支持懒加载，并与一体式 Widget 共用悬浮、触屏、公开分析跳转和响应式尺寸行为。", "embed_code": "嵌入代码", "resource_url": "资源地址", "map_control": "地图控件代码", "recent_records": "最近记录", "filter_page_export": "筛选、分页与导出", "collection_method": "采集方式", "all_methods": "全部方式",
	...
	"en": {
		...
		"url_overrides": "URL parameters can override defaults", "preset_preview": "Live Map Preset preview", "preview_mode": "Preview mode", "interactive_widget_preview": "Interactive Widget", "image_fallback": "Image fallback", "width": "Width", "height": "Height", "auto_width": "Calculate width from current options",
		...
		"save_settings": "Save Site settings", "combined_widget": "Integrated", "separate_tracker": "Separated", "widget_resource": "Widget", "javascript_widget": "Interactive Widget", "image_widget": "Image Widget", "recommended": "Recommended", "interactive": "Interactive", "compatibility_mode": "Compatibility", "image_widget_limitations": "No JavaScript required. It creates no browser Visitor ID and uses Referer for the default path. Image proxies, prefetching, or caches may render without counting; add a path parameter for a fixed page.", "tracker_resource": "Tracker", "map_resource": "Interactive map", "separated_widget_help": "Display only; it records no Pageview. It lazy-loads the same hover, touch, Public Analytics navigation, and responsive sizing experience as the Integrated Widget.", "embed_code": "Embed code", "resource_url": "Resource URL", "map_control": "Map control code", "recent_records": "Recent records", "filter_page_export": "Filter, paginate, and export", "collection_method": "Collection method", "all_methods": "All methods",
