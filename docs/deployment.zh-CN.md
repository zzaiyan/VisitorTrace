...
脚本会创建或复用专用服务账户，创建受保护目录；在配置不存在时执行 `init`；初始化自更新稳定路径；写入 systemd 单元并启动服务。在 `ProtectSystem=strict` 下，生成的 unit 会将数据目录和整个配置目录同时加入 `ReadWritePaths`。初始化时会提示输入管理员密码。脚本不会下载二进制或 GeoIP 文件，不会创建备份，也不会配置反向代理或宝塔。需要逐项检查命令时，再使用下面的手动步骤。
