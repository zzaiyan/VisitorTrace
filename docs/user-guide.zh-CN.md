...
正式版本在 GitHub Releases 提供带版本号且无需 Go 环境的 Linux 二进制。根据服务器架构选择 `visitortrace-<版本>-linux-amd64` 或 `visitortrace-<版本>-linux-arm64`，同时下载 `checksums.txt`。例如，校验 `0.2.2` 的 AMD64 版本：
...
实际安装时将 `0.2.2` 替换为下载的版本号；ARM64 服务器使用 `linux-arm64` 文件名。每个 Release 还会提供 GPL 文本和来自同一标签的对应源码归档。Release 清单另有 Ed25519 签名，供内置自更新器验证；手工安装时仍应先核对 `checksums.txt`。使用发布版时，后续示例中的 `./bin/visitortrace` 对应 `$HOME/.local/bin/visitortrace`。
...
每个 Site 都有独立的 Site ID、Allowed Origins、统计时区、访客合并周期、逐条记录保留策略和 Map Preset。
...
逐条记录可选择 1 至 90 天的“限时”保留，或选择“无限”。选择无限后，天数输入变为只读但会记住最近的数值，切回限时时可继续使用。无限保留不会删除任何 Pageview Record，因此需要留意 SQLite 占用和服务器剩余磁盘空间。

使用 CLI 创建 Site 时可增加 `--retention-unlimited`；可选的 `--retention` 数值会作为以后切回限时模式的备用值保留。
...
- 实际年龄超过限时保留期的 Pageview Record；无限保留的 Site 会跳过该项；
- 已经结束的访客合并窗口登记；
- 过期或超过 12 小时未活动的管理员 Session。

删除采用有上限的小批次事务，避免长时间阻塞采集。每日聚合和地图统计不会随逐条记录过期而删除。缩短保留期或从无限切回限时，会让新超出范围的记录在下一轮维护中被清理；延长保留期或切换到无限都不能恢复已经删除的记录。无限只作用于 Pageview Record，已经结束的访客窗口登记和过期管理员 Session 仍会正常删除。
...
管理总览会列出本机 `.vtbackup` 归档，并提供“恢复备份”功能。选择归档、输入当前管理员密码并确认后，VisitorTrace 会校验归档旁的校验文件和归档内容，在 `backup_dir/pre-restore` 创建新的恢复前安全快照，并安排在下一次受管重启时替换数据库。当前配置文件不会被覆盖；恢复成功后所有管理员会话都会失效，服务就绪后需要重新登录。
