...
后台支持尺寸、标题、PV/UV 标签、字体大小、显示内容、背景、陆地、边界、文字、标记颜色和标记指标。实时预览可在完整交互式 Widget 与 Image fallback 之间切换，无需先保存；Widget 预览与正式接入一致，支持点位详情、指针/触屏平移、滚轮/双指缩放、Reset 和分析页跳转。私有 Site 通过管理员专用端点获得相同行为。宽度和高度旁的自动按钮会根据当前标题、统计栏和字体大小计算保持世界地图投影比例所需的另一维度。
