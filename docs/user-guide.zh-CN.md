...
正式版本在 GitHub Releases 提供带版本号且无需 Go 环境的 Linux 二进制。根据服务器架构选择 `visitortrace-<版本>-linux-amd64` 或 `visitortrace-<版本>-linux-arm64`，同时下载 `checksums.txt`。例如，校验 `0.1.3` 的 AMD64 版本：
...
实际安装时将 `0.1.3` 替换为下载的版本号；ARM64 服务器使用 `linux-arm64` 文件名。每个 Release 还会提供 GPL 文本和来自同一标签的对应源码归档。Release 清单另有 Ed25519 签名，供内置自更新器验证；手工安装时仍应先核对 `checksums.txt`。使用发布版时，后续示例中的 `./bin/visitortrace` 对应 `$HOME/.local/bin/visitortrace`。
...
Site 页面为每段接入代码和每个资源地址提供一键复制。一体式接入成对展示 Widget 嵌入代码与脚本地址；分离式接入分别为 Tracker 和懒加载 Map SVG 展示嵌入代码与资源地址。
