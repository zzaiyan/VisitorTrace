...
“管理员设置 > GeoIP 数据库”提供定位后端、更新策略、官方源/自定义镜像、可选校验地址和后端凭证管理。已保存的秘密不会回显到浏览器：凭证输入框留空表示保持原值，显式勾选清除项才会删除。保存 GeoIP 设置需要管理员密码，并会重启受管服务。同一区域可以检查当前数据库，也可以在“仅手动”策略下强制重新下载。
...
已有安装不需要重新执行 `init` 即可切换后端：进入“管理员设置 > GeoIP 数据库”，按需填写新凭证并保存，服务会使用新后端重启。其他后端已经保存的凭证会继续保留，除非显式清除。
