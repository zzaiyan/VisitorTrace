...
- 管理后台新增本地 `.vtbackup` 备份恢复：先校验归档并创建恢复前安全快照，在下一次受管重启时安全替换数据库，保留当前配置，并在恢复成功后撤销管理员 Session。
- 默认开发/构建版本推进到 `0.2.2-dev`，并增加基于 Git 忽略本地凭证文件的可选 GeoIP 集成测试，可验证已有后端数据库，并按需验证官方下载、解包、结构校验、激活和查询。
