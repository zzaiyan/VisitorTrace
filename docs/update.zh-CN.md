...
- 管理员设置新增本地 `.vtbackup` 备份恢复：先校验归档并创建恢复前安全快照，在下一次受管重启时安全替换数据库，保留当前配置，并在恢复成功后撤销管理员 Session。
- Map Preset 的保存按钮旁新增“恢复默认”。该操作只重置尚未保存的表单，并立即刷新两种预览和 URL 参数示例，不写入数据库。
- 在 `0.2.2` 发布后推进默认开发/构建版本，并增加基于 Git 忽略本地凭证文件的可选 GeoIP 集成测试，可验证已有后端数据库，并按需验证官方下载、解包、结构校验、激活和查询。本版本继续使用 SQLite Schema 12。
