...
- 以 Step-up 验证取代逐操作的管理员密码输入：密码校验为会话开启 5 分钟窗口，登录本身即开窗，敏感操作在窗口内直接执行；窗口过期时请求会被质询，AJAX 导航器弹出统一的验证对话框并自动续发原请求，无 JavaScript 渲染的表单仍内联密码框。
- 破坏性操作分级防护：清空 Site 数据增加确认对话框，永久删除 Site 额外要求输入 Site ID，修改密码明示全部会话将失效。
- 增加操作反馈：所有 POST 表单在请求进行中显示按钮忙态指示；备份、清理和 GeoIP 更新纳入原地 AJAX 表单；结果以操作语境附近的自动消退 toast 呈现，替代页面顶部的闪现条。
- 改密码表单重排为等宽三列并合并提示为单行；修复 step-up 布局变化后配置操作行与危险区按钮的尺寸异常。
- 本版本保持 SQLite Schema 12。
...
- 键盘与读屏焦点在 body 交换后保持：优先回到同 id 的原焦点元素，否则回到触发导航的链接。
- 本地化约一百条后台错误与提示文案，覆盖错误页、重定向提示、明细筛选校验、登录流程和 GeoIP 凭证检查，日语和英文界面不再回退显示中文。
- 本版本保持 SQLite Schema 12。

## 0.2.5 - 2026-09-24

- 后台与公开分析页引入基于 fetch 的导航：同源链接点击和 GET 表单提交改为原地交换页面内容，切换统计范围、语言或访问明细筛选时保持滚动位置，前进/后退按各历史条目还原偏移。POST 提交、下载和无所需 API 的浏览器保持整页行为。
- 交互式地图的缩放与平移在范围和语言切换后保持不变；切换 Site、整页刷新和 Reset 控件仍从默认视图开始。
- 本版本保持 SQLite Schema 12。

## 0.2.4 - 2026-09-24

- 将所有分区 eyebrow 统一为固定的简短英文同义回声，改由 CSS 以大写显示，不再跟随界面语言；此前部分小字走翻译、部分为硬编码全大写英文，多语言页面观感不一致。
- 本地化此前硬编码的后台页面标题，覆盖访问明细、管理员设置、新增 Site 和各重启过渡页，浏览器标签页标题现在跟随所选界面语言。
- 本版本保持 SQLite Schema 12。
