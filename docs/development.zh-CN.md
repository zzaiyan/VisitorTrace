...
GeoIP Resolver 每次只打开配置中的一个后端。DB-IP 和 MaxMind 使用嵌套的 country/subdivision/city/location MMDB 结构；IP2Location 使用扁平的 country/region/city/latitude/longitude 结构。三者都会映射为相同的 `geoip.Location` 字段。`ValidateWithProvider` 按后端执行 MMDB 校验，归因信息也根据当前后端选择。更新器在启动时和每 24 小时运行，`{YYYY-MM}` 使用 UTC 月份展开。压缩输入限制为 1 GiB，展开后的 MMDB 限制为 2 GiB。配置 SHA-256 sidecar 时先校验下载容器；随后始终调用 MMDB 完整验证。候选文件与目标位于同一文件系统，通过重命名激活，上一版保留为 `.previous`。服务通过互斥保护的 Resolver 热交换，避免关闭仍在查询的旧句柄。

DB-IP City Lite 的 `city.names` 可能包含城市和区、街道等限定词，而 Lite Schema 不提供可用于选择行政层级的 feature code。`internal/geoip.CityLevelName` 会清理中国地名中的限定词；当 DB-IP 在括号逗号后提供城市成分时优先使用该成分，并对北京、上海、天津、重庆使用上级直辖市名称。该规范化只作用于 DB-IP；MaxMind 和 IP2Location 的城市名称在结构映射后保持原值。
