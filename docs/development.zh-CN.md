...
DB-IP City Lite 的 `city.names` 可能包含城市和区、街道等限定词，而 Lite Schema 不提供可用于选择行政层级的 feature code。`internal/geoip.CityLevelName` 会清理中国地名中的限定词；当 DB-IP 在括号逗号后提供城市成分时优先使用该成分，并对北京、上海、天津、重庆使用上级直辖市名称。结果用于新的 Pageview 地理位置和地图，不宣称街道级精度。
