...
`internal/operations` 聚合只读运行信息：构建元数据、进程运行时长、SQLite/WAL/SHM 占用、文件系统空间、GeoIP 与本地备份状态，以及 `operation_status` 中的任务结果。Linux 使用 `statfs`，其他平台明确返回不可用而不伪造数值。后台手动操作复用 CLI 相同的备份、维护和 GeoIP 实现，并受管理员 Session 与 CSRF 保护。GeoIP 设置路由还会重新验证管理员密码、保留未提交的新秘密，并在请求受管重启前原子保存受保护配置。人工 GeoIP 检查会临时启用 Runner，不改变已经保存的“仅手动”策略。GeoIP 和维护 Runner 使用进程级互斥避免重复执行。
