...
The SVG renderer stores the basemap once in `<defs>` and places the two Bering-seam copies with `<use>`. A clipped `visitortrace-map-content` group contains both the basemap and `visitortrace-marker-layer`, so they share one transform while remaining isolated from the title and statistics bands. Marker groups retain native `<title>` elements for standalone SVGs and add escaped city/country/PV/UV attributes for interactive consumers. `GET /embed/widget?site_id=...` renders that SVG inline in a cacheable, sandbox-compatible HTML document with no external runtime dependencies. The document supplies pointer, focus, touch details, pointer/touch pan and zoom, a lower-left control registry, links to Public Analytics, and validated preset dimensions through `postMessage`. Pan and zoom writes are coalesced with `requestAnimationFrame`; pointer-capture loss and window blur clear gesture state, while touch gestures that start on a marker defer capture until movement exceeds the tap threshold. It never ingests a Pageview. `widget.js` collects the parent hostname/path and mounts the lazy iframe, while the separated `tracker.js` and `map.js` endpoints enable those two responsibilities independently. Both display loaders accept resize messages only from their own iframe's `contentWindow`.

Interactive map controls use the same extension contract in the iframe and ECharts views. `window.VisitorTraceMapControls.add({id, label, icon, activate})` registers a control, while `remove(id)` removes it. ECharts map elements also expose `element.visitorTraceMapControls` and emit the bubbling `visitortrace:map-controls-ready` event. The built-in Reset action is the first consumer of this interface. The Administrator-only `/admin/sites/{siteID}/preset-preview` endpoint renders the same iframe document from `AdminMapData`, allowing live preview for private Sites without weakening public publication checks.
...
```sh
git tag -a v0.1.4 -m "VisitorTrace v0.1.4"
...
go run ./tools/release-manifest generate \
  ...
  --asset linux-amd64=dist/visitortrace-0.1.4-linux-amd64 \
  --asset linux-arm64=dist/visitortrace-0.1.4-linux-arm64 \
