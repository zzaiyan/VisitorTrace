...
Manual record geolocation refresh holds the Store write lock and one SQLite transaction while it rereads retained Pageview rows in batches. It updates record geography, then rebuilds only country, region, and city aggregates for Site-local dates that still have at least one detailed row. UV reconstruction uses the deduplication rule effective on each row's date and the stored hostname scope. Dates without retained rows and all non-geographic aggregates remain unchanged. Existing visitor registrations are preserved; registrations reconstructed for active geographic windows are added so later ingestion does not recount them. Invalid stored IPs retain their old geography, while valid addresses without a current match are cleared. The HTTP layer pins one Resolver under `geoMu`, excludes concurrent refreshes, and invalidates the Site map cache only after commit.

Public and administrative aggregate queries share Site-local date boundaries. Public queries first enforce publication state and never read the path family; authenticated administrative queries can read path aggregates even when publication is disabled. The Site detail page obtains its all-history map through `AdminMapData`, independent of publication state, and sends only map points and labels to the browser. The analytics bundle initializes trend and map charts independently so this map-only view reuses the same ECharts implementation as Public Analytics. Browser JSON is generated only from already-authorized aggregates and contains no individual records, original IPs, or Visitor Digests.
...
`serve` starts a lightweight maintenance loop that runs on startup and hourly. `visitortrace maintenance` exposes the same cleanup flow for diagnostics and external schedulers. Every deletion transaction has a bounded batch size; `operation_status` retains the latest maintenance outcome for the operational dashboard. A Site with `retention_unlimited` skips only Pageview Record deletion. Its completed visitor registrations are still cleaned, and the global Administrator-session cleanup always runs.
...
The SVG renderer stores the basemap once in `<defs>` and places the two Bering-seam copies with `<use>`. A clipped `visitortrace-map-content` group contains both the basemap and `visitortrace-marker-layer` while isolating them from the title and statistics bands. Marker groups retain native `<title>` elements for standalone SVGs and add escaped city/country/PV/UV attributes for interactive consumers. `GET /embed/widget?site_id=...` renders that SVG inline in a cacheable, sandbox-compatible HTML document with no external runtime dependencies. The document supplies pointer, focus, and touch marker details, links to Public Analytics, and validated preset dimensions through `postMessage`; it intentionally has no pan, zoom, Reset control, or map-control registry. The frame renderer derives the Public Analytics route itself for both deployed and Administrator-preview Widgets. Native anchor drag is disabled, while a movement threshold cancels the resulting click without pointer capture or map transforms. The frame never ingests a Pageview. `widget.js` collects the parent hostname/path and mounts the lazy iframe, while the separated `tracker.js` and `map.js` endpoints enable those two responsibilities independently. Both display loaders accept resize messages only from their own iframe's `contentWindow`.

ECharts analysis maps outside the Widget expose `element.visitorTraceMapControls`, emit the bubbling `visitortrace:map-controls-ready` event, and use the built-in Reset action as the first consumer of that extension contract. The lightweight iframe Widget does not expose this interface. The Administrator-only `/admin/sites/{siteID}/preset-preview` endpoint renders the same Widget document from `AdminMapData`, allowing live preview for private Sites without weakening public publication checks.
...
```sh
git tag -a v0.2.0 -m "VisitorTrace v0.2.0"
...
go run ./tools/release-manifest generate \
  ...
  --asset linux-amd64=dist/visitortrace-0.2.0-linux-amd64 \
  --asset linux-arm64=dist/visitortrace-0.2.0-linux-arm64 \
