...
GitHub Releases provide versioned Linux executables that do not require a Go toolchain. Select `visitortrace-<version>-linux-amd64` or `visitortrace-<version>-linux-arm64` for the server architecture, download `checksums.txt` beside it, and verify it. For example, for version `0.2.0` on AMD64:

```sh
grep ' visitortrace-0.2.0-linux-amd64$' checksums.txt | sha256sum -c -
install -Dm700 visitortrace-0.2.0-linux-amd64 "$HOME/.local/bin/visitortrace"
...
sudo ./scripts/update-systemd-binary.sh \
  --binary ./visitortrace-0.2.0-linux-amd64 \
